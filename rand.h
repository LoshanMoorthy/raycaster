...
// rotate left
static __forceinline u64 rotl(const u64 x, int k) {
    return (x << k) | (x >> (64 - k));
}

// create new rand with specified seed
#define rand_create(_seed) ({ Rand __r; rand_seed(&__r, (_seed)); __r; })
...
// set seed _seed for Rand *_prand
...
#define rand_n(_prand, _min, _max)                                       \
    ({                                                                   \
        ...
        const u64                                                        \
            __r =                                                        \
                rotl(__prand->s[0] + __prand->s[3], 23) + __prand->s[0], \
            __t = __prand->s[1] << 17;                                   \
        ...
        __prand->s[2] ^= __t;                                            \
        __prand->s[3] = rotl(__prand->s[3], 45);                         \
